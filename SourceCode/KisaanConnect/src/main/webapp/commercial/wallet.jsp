...
<head>
    ...
    <title>Commercial Organisation Wallet | KisaanConnect</title>
    ...
    <div class="main-content">
        ...
        <div class="dashboard-content" style="padding: 30px;">
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 15px; margin-bottom: 24px;">
                <div>
                    <h1 style="font-size: 26px; color: #1F2937; margin-bottom: 4px;"><i class="fa-solid fa-wallet"></i> Corporate Procurement Wallet</h1>
                    <p style="color: #6B7280; font-size: 14px;">Pre-fund bulk procurement orders, track transactions, and manage organization balances.</p>
                </div>
                <button onclick="window.print();" class="no-print" style="background: #1E40AF; color: white; border: none; padding: 10px 20px; border-radius: 10px; font-weight: 600; cursor: pointer; display: inline-flex; align-items: center; gap: 8px;">
            ...
            <div class="balance-card">
                <div>
                    <span style="opacity: 0.85; font-size: 14px; text-transform: uppercase; letter-spacing: 0.5px;">Corporate Wallet Balance</span>
                    <h2 style="font-size: 38px; font-weight: 800; margin: 6px 0;">₹<%= wallet != null ? wallet.getCurrentBalance() : "0.00" %></h2>
                    <div style="font-size: 13px; opacity: 0.9;">Total Procurements Paid: ₹<%= wallet != null ? wallet.getTotalDebited() : "0.00" %></div>
                </div>

                <div>
                    <form action="${pageContext.request.contextPath}/wallet/topup" method="post" style="display: flex; gap: 8px; margin: 0;">
                        <input type="number"
                               name="amount"
                               placeholder="Enter ₹ amount"
                               min="100"
                               step="100"
                               style="height: 48px; border-radius: 12px; border: none; padding: 0 16px; font-weight: 600; font-family: var(--kc-font);"
                               required>
                        <button type="submit" style="background: white; color: #1E3A8A; border: none; padding: 0 20px; border-radius: 12px; font-weight: 700; cursor: pointer;">
            ...
            <!-- Transaction Ledger -->
            <div class="txn-card">
                <h3 style="font-size: 18px; color: #1F2937; margin-bottom: 16px;">Corporate Disbursement Ledger</h3>

                <% if (transactions == null || transactions.isEmpty()) { %>
                    <p style="text-align: center; color: #6B7280; padding: 30px 0;">No corporate wallet transactions yet.</p>
                <% } else { %>
